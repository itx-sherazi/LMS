...
import { getCurrentCourseProgressService, markLectureAsViewedService, resestCoursProgressService } from "@/services";
...
function StudentViewCourseProgressPage() {
    ...
    async function handleRewatchCourse(){
      const response = await resestCoursProgressService(auth?.user?._id,studentCurrentCourseProgress?.courseDetails?._id)
      if(response?.success){
    ...
    async function updateCourseProgress() {
      if (currentLecture) {
        ...
        if (response?.success) {
          fetchCurrentCourseProgress(); // Existing fetch function
          
          // **Auto Move to Next Lecture Logic**
          ...
          if (currentIndex !== -1 && currentIndex < allLectures.length - 1) {
            setCurrentLecture(allLectures[currentIndex + 1]); // Move to next lecture
          } else {
            setshowCourseCompleteDialog(true);
            setShowConfetti(true);
          }
              ...
              <ScrollArea className="h-full">
                <div className="p-4">
                  ...
                  <Dialog open={showCourseCompleteDialog}>
                    <DialogContent showOverly={false} className="sm:w-[425px]">
                        <DialogHeader>
                            <DialogTitle>
                                Congratulations! You've completed this course
                            </DialogTitle>
                            <DialogDescription className="flex flex-col gap-3">
                                <Label>Keep up the great work and spread positivity!</Label>
